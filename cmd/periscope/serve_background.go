...
func reportBackgroundLaunchInProgress(dataDir, authToken string) {
	...
	fmt.Println("periscope serve --background is already in progress.")
}
...
func runServeBackground(
	...
	fmt.Printf(
		"periscope starting in background (pid %d)\n",
		result.childPID,
	)
	fmt.Printf("Logs: %s\n", result.LogPath)
}
...
func validateUniqueWritableDaemonSet(dataDir, authToken string) error {
	...
	return fmt.Errorf(
		"multiple writable periscope daemons are running (pids %s); refusing startup or replacement; run `periscope daemon status`, then `periscope daemon stop` before retrying",
...
func ensureBackgroundServe(
	...
	for {
		...
		if _, err := findIncompatibleWritableDaemonRuntime(
			...
			return nil, fmt.Errorf(
				"incompatible daemon is already running: %w; run "+
					"`periscope daemon stop` before starting this version",
				err,
			)
		}
		if IsLocalDaemonActive(cfg.DataDir, cfg.AuthToken) {
		...
		return nil, fmt.Errorf(
			"periscope serve --background did not publish a runtime record",
		)
	}
	defer func() { _ = launchLock.Unlock() }()
...
func waitForExternalServeStartup(
	...
	return nil, true, fmt.Errorf(
		"periscope serve startup finished without publishing a writable " +
			"runtime record",
	)
}
...
func startServeBackgroundProcess(
	...
	if _, err := fmt.Fprintf(
		logFile,
		"\n--- periscope serve background start %s ---\n",
