...
func runExportSessions(cmd *cobra.Command, cfg exportSessionsConfig) error {
	...
	if backfill.State != "not_needed" && backfill.State != "completed" {
		return fmt.Errorf(
			"project identity evidence backfill is %s (%d/%d); start or restart the writable daemon, then check `periscope export status`",
	...
	if err != nil {
		if errors.Is(err, db.ErrDatabaseIDMissing) {
			return fmt.Errorf(
				"database id missing; restart periscope serve to initialize export metadata: %w",
				err,
			)
		}
		return err
