...
func runDuckDBStatus() {
	...
	if status.MirrorMissing {
		fmt.Printf(
			"DuckDB mirror not found at %s; run 'periscope duckdb push' to create it\n",
			duckCfg.Path,
		)
		return
	}
...
func runDuckDBServe(appCfg config.Config, basePath string) {
	...
	if rt.PublicURL == rt.LocalURL {
		fmt.Printf(
			"periscope %s (duckdb read-only) at %s\n",
			version,
			rt.LocalURL,
		)
	} else {
		fmt.Printf(
			"periscope %s (duckdb read-only) backend at %s, public at %s\n",
			version,
			rt.LocalURL,
			rt.PublicURL,
		)
	...
	if schemaErr != nil {
		if duckCfg.URL == "" {
			fatal("duckdb serve: schema incompatible: %v\n"+
				"Run 'periscope duckdb push --full' to repopulate the mirror.", schemaErr)
		}
...
func duckDBMirrorServeProbeError(probe duckdbsync.MirrorProbe) error {
	...
	return fmt.Errorf("%s; rebuild with 'periscope duckdb push --full'", reason)
}
...
func identifyQuackNode(ctx context.Context, conn *sql.DB, machine string) {
	...
	_, err := conn.ExecContext(ctx,
		`CALL quack_identify(?, ?, ?, ?, ?)`,
		"periscope", "periscope", machine, "", meta,
	)
	if err != nil {
