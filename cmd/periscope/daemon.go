...
func reportDaemonLaunchContention(
	...
	if len(observation.Records) > 1 {
		return fmt.Errorf(
			"daemon start: multiple writable periscope daemons are running (pids %s); run `periscope daemon status`, then `periscope daemon stop` before retrying",
			formatRecordPIDList(observation.Records),
		)
	}
	if len(observation.Records) > 0 {
		rt := daemonRuntimeFromRecord(observation.Records[0])
		fmt.Fprintf(w, "periscope already running at %s (pid %d)\n", urlFromDaemonRuntime(rt), rt.Record.PID)
		return nil
	}
	if observation.Starting {
		return daemonPersistentStartupError("daemon start", dataDir, observation.Snapshot, now)
				...
				for _, rec := range confirmed {
					if !deps.probe(rec, cfg.AuthToken) {
						...
						return daemonLaunchObservation{Err: fmt.Errorf(
							"writable daemon pid %d is not responding to its health probe; run `periscope daemon restart` to replace it",
							rec.PID,
						)}
					}
					if compatibilityErr := daemonRuntimeCompatibilityError(
						...
						_ = lock.Unlock()
						return daemonLaunchObservation{Err: fmt.Errorf(
							"writable daemon pid %d is incompatible: %w; run `periscope daemon restart` to replace it",
...
func daemonSlowStartupError(
	...
	return fmt.Errorf(
		"%s: startup is still in progress (%s); the child continues running; retry `periscope daemon status`",
		operation, strings.Join(details, ", "),
	)
}
...
func (p *daemonLaunchProgressWriter) launch(pid int, logPath string) {
	fmt.Fprintf(p.w, "Starting periscope (pid %d)...\n", pid)
	if logPath != "" {
...
func runDaemonStatus(w io.Writer, deps daemonCommandDeps) error {
	...
	if len(writable) > 1 {
		fmt.Fprintln(w, "warning: multiple writable periscope daemons are running; the single-writer invariant is violated.")
	...
	if deps.isStarting(cfg.DataDir) {
		fmt.Fprintln(w, "periscope daemon is starting up.")
		for _, line := range serveStartingStatusLines(deps.readStartupState(cfg.DataDir), deps.now()) {
			fmt.Fprintln(w, line)
		}
		return nil
	}
	fmt.Fprintln(w, "No periscope daemon is running.")
	return nil
}
...
func writeDaemonRecordStatus(
	...
	if compatErr != nil {
		fmt.Fprintln(w, "periscope found an incompatible running writable daemon.")
		for _, line := range serveStatusLines(rt) {
			fmt.Fprintln(w, line)
		}
	...
	if !responding {
		fmt.Fprintln(w, "periscope daemon process is running but not responding to health checks.")
		for _, line := range serveStatusLines(rt) {
			fmt.Fprintln(w, line)
		}
		if rec.SourcePath != "" {
...
func runDaemonStop(w io.Writer, deps daemonCommandDeps) error {
	...
	if len(records) == 0 {
		fmt.Fprintln(w, "periscope daemon is not running.")
		return nil
	}
...
func runDaemonRestartWithPolicy(
	...
	if wasRunning {
		...
		fmt.Fprintln(w, "periscope started (was not running).")
		if result.Runtime != nil {
			fmt.Fprintf(w, "periscope running at %s (pid %d)\n", urlFromDaemonRuntime(result.Runtime), result.Runtime.Record.PID)
		}
		if result.LogPath != "" {
			fmt.Fprintf(w, "Logs: %s\n", result.LogPath)
		}
...
func daemonWaitCanceledError(operation string, result backgroundLaunchResult) error {
	...
	return fmt.Errorf(
		"%s: wait canceled (%s); the child continues running; run `periscope daemon status` to inspect it",
		operation, strings.Join(details, ", "),
	)
}
