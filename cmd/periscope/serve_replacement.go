...
func prepareForegroundServeDaemon(
	...
	case serveReplacementUseExisting:
		...
		if rt != nil {
			fmt.Printf(
				"periscope already running at %s (pid %d)\n",
	...
	case serveReplacementAuto, serveReplacementExplicit:
		...
		if !ownsStartLock {
			...
			return false, noRelease, fmt.Errorf(
				"periscope serve startup is already in progress; " +
					"wait for it to finish or run `periscope daemon status`",
			)
		}
		fmt.Println("Replacing periscope daemon")
...
func acquireForegroundServeLaunchLock(cfg config.Config) (func(), error) {
	...
	if !ok {
		return nil, fmt.Errorf(
			"periscope serve --background is already in progress; " +
				"wait for it to finish or run `periscope daemon status`",
		)
	}
...
func serveDaemonConflictLines(decision serveReplacementDecision) []string {
	lines := serveDaemonDecisionLines(
		"periscope found a running writable daemon that will not be replaced.",
		decision,
	)
	...
	case serveReplacementRefuse:
		return append(lines,
			"Run `periscope daemon restart` to restart it from config.toml, "+
				"`periscope serve --replace` to replace it with these serve "+
				"flags, or `periscope daemon stop` to stop it first.",
		)
	case serveReplacementUseExisting:
		return append(lines,
			"Using the existing daemon. Run `periscope daemon stop` "+
				"to stop it first.",
		)
	default:
		return append(lines,
			"Run `periscope daemon stop` to stop it first.",
		)
	}
}
...
func serveDaemonReplacementLines(decision serveReplacementDecision) []string {
	lines := serveDaemonDecisionLines(
		"periscope will replace a running writable daemon.",
		decision,
	)
	if decision.Runtime == nil {
		return lines
	...
	return append(lines,
		"Run `periscope daemon stop` to stop it manually instead.",
	)
}
...
func serveDaemonDecisionLines(
	...
	rt := decision.Runtime
	if rt == nil {
		return []string{"No writable periscope daemon is running."}
	}
	lines := []string{
		header,
