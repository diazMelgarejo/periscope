...
func TestTranslateRecallSearchErrorUsesRecallRemediation(t *testing.T) {
	...
	t.Run("building", func(t *testing.T) {
		...
		assert.NotContains(t, err.Error(), "periscope embeddings build",
...
func TestRecallSchedulerBackstopRemovesArchivedEntryVectors(t *testing.T) {
	dataDir := t.TempDir()
	database := dbtest.OpenTestDBAt(t, filepath.Join(dataDir, "sessions.db"))
	dbtest.SeedSession(t, database, "s1", "periscope")
...
{"candidate_id":"second-import","type":"fact","scope":"repository","title":"Import policy","body":"Keep imports manual too.","project":"periscope","agent":"codex","session_id":"import-session","label":"correct","transferable":true,"provenance_ok":true,"evidence":{"ordinal_start":0,"ordinal_end":0}}
`)
	req, err := http.NewRequestWithContext(
...
func TestRecallImportSchedulesEmbeddingRefresh(t *testing.T) {
	...
	ts := startTestServer(t, ln, srv.Handler())
	input := strings.NewReader(`
{"candidate_id":"imported-entry","type":"fact","scope":"repository","title":"Database pool","body":"Reuse idle connections.","project":"periscope","agent":"codex","session_id":"import-session","label":"correct","transferable":true,"provenance_ok":true,"evidence":{"ordinal_start":0,"ordinal_end":0}}
`)
	req, err := http.NewRequestWithContext(
		t.Context(), http.MethodPost,
		ts.URL+"/api/v1/recall/import?allow_placeholder_sessions=true", input,
