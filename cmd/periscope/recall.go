...
func newRecallImportCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if !dryRun && !yes {
				return fmt.Errorf(
					"recall import writes to the active periscope database; " +
	...
	cmd.Flags().BoolVar(
		...
		"yes",
		false,
		"Confirm importing reviewed entries into the active periscope database",
	)
	cmd.Flags().BoolVar(
		&allowRemoteImport,
		"allow-remote-import",
	...
	cmd.Flags().BoolVar(
		...
		false,
		"Allow validating or importing reviewed entries against a default periscope data directory",
	)
	cmd.Flags().BoolVar(
...
func requireSafeLocalRecallImportTarget() error {
	...
	if err != nil {
		return fmt.Errorf("resolving periscope data directory: %w", err)
	}
	...
	return fmt.Errorf(
		"recall import refuses to validate or write against the default periscope data directory %s; "+
