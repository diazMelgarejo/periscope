...
func TestServeCheckDataVersionRejectsNewerDatabase(t *testing.T) {
	...
	assert.NotContains(t, err.Error(), `Run "periscope update"`)
}
...
func TestRootVersionFlag(t *testing.T) {
	...
	require.NoError(t, err, "Execute")
	assert.Contains(t, got, "periscope ", "version output = %q", got)
}
...
func TestVersionJSONContractDoesNotRequireRuntimeState(t *testing.T) {
	...
	assert.Equal(t, "periscope", doc.Name)
...
func TestExecuteCLIWithLegacyFlagCompatWarnsOnce(t *testing.T) {
	...
	require.NoError(t,
		...
		"Execute")
	assert.Contains(t, stdout.String(), "periscope ",
