...
func TestValidateServiceSpec_RejectsUnsafeChars(t *testing.T) {
	const clean = "/usr/local/bin/periscope"
	...
	spaced := cleanSpec
	spaced.DataDir = "/home/me/App Support/periscope"
...
func TestLaunchdRender(t *testing.T) {
	...
	spec := serviceSpec{
		BinPath: "/usr/local/bin/periscope",
	...
	wants := []string{
		`<string>periscope.pg-watch</string>`,
		`<string>/usr/local/bin/periscope</string>`,
		`<string>pg</string>`,
		`<string>push</string>`,
		`<string>--watch</string>`,
...
func TestLaunchdUnitPath(t *testing.T) {
	...
	want := filepath.Join(
		"/Users/me", "Library", "LaunchAgents", "periscope.pg-watch.plist",
	)
	if m.unitPath() != want {
...
func TestLaunchdStop_BootsOut(t *testing.T) {
	...
	if !rr.sawContains("launchctl bootout gui/501/periscope.pg-watch") {
...
func TestSystemdRender_Golden(t *testing.T) {
	...
	got := m.render(spec)
	want := `[Unit]
Description=periscope PostgreSQL auto-push
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
ExecStart="/usr/local/bin/periscope" pg push --watch
...
func TestSystemdUnitPath(t *testing.T) {
	m := &systemdManager{user: "me", home: "/home/me"}
	want := filepath.Join(
		"/home/me", ".config", "systemd", "user", "periscope-pg-watch.service",
	)
	if m.unitPath() != want {
		t.Errorf("unitPath = %q, want %q", m.unitPath(), want)
	}
...
func TestSystemdInstall_ReloadsAndEnables(t *testing.T) {
	...
	if !rr.sawContains("systemctl --user enable --now periscope-pg-watch.service") {
...
func TestSystemdStart_CallsStart(t *testing.T) {
	...
	if !rr.sawContains("systemctl --user start periscope-pg-watch.service") {
...
func TestSystemdStop_CallsStop(t *testing.T) {
	...
	if !rr.sawContains("systemctl --user stop periscope-pg-watch.service") {
...
func TestSystemdUninstall_DisablesAndRemoves(t *testing.T) {
	...
	if !rr.sawContains("systemctl --user disable --now periscope-pg-watch.service") {
