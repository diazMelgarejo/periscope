// ABOUTME: `periscope mcp` subcommand — serves the read-only MCP tools
...
the local periscope daemon, starting it when needed so a long-lived MCP server
...
func (s *mcpDaemonService) daemonService(
	...
	if tr.Mode != transportHTTP {
		return nil, errors.New(
			"periscope mcp requires a daemon; refusing direct archive access",
		)
	}
