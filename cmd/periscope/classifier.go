// ABOUTME: `periscope classifier rebuild` — clears the
	...
	if tr.Mode == transportHTTP {
		return errors.New(
			"local daemon is serving on " + tr.URL +
				"; stop 'periscope serve' or the read-only mirror serve " +
...
func runClassifierRebuild(
		...
		if err := clearPGClassifierHash(ctx, cfg, pgCfg); err != nil {
			return fmt.Errorf(
				"clearing PG hash: %w (SQLite was cleared "+
					...
					"'periscope classifier rebuild', or run "+
					"'periscope pg push --full' to repopulate "+
	...
	fmt.Fprintln(out,
		"restart any running 'periscope serve' so write "+
