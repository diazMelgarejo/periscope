...
import (
	...
	"github.com/latentsignal-org/periscope/internal/telemetry"
...
func runServe(cfg config.Config, opts serveOptions) {
	...
	if rt.PublicURL == rt.LocalURL {
		fmt.Printf(
			"periscope %s listening at %s (started in %s)\n",
			version, rt.LocalURL,
			time.Since(start).Round(time.Millisecond),
		)
	} else {
		fmt.Printf(
			"periscope %s backend at %s, public at %s (started in %s)\n",
...
func runDeferredStartupSyncFallback(
	...
	// A foreground `periscope sync` may have already driven startup
...
// `periscope sync` never runs the archive-scale pass in the daemon. A failure
...
func daemonRestartUpgradeHint() string {
	return "This database was written by an older periscope version and " +
		...
		"  - CLI: run `periscope daemon restart`"
}
...
func rejectLiveWritableDaemonBeforeDirectWrite(cfg config.Config) error {
	...
	return fmt.Errorf(
		"%s; refusing to write directly. Retry through the daemon or "+
			"run `periscope daemon stop` first",
		reason,
	)
}
