...
// ABOUTME: periscope server using its kit daemon runtime record.
package main
...
func stopWritableDaemonRecordsSafely(
	...
	if len(unconfirmed) > 0 {
		return fmt.Errorf(
			"cannot confirm every writable periscope daemon: %s; no process was signalled; verify each process and terminate it manually before retrying",
	...
	for i, rec := range records {
		...
		fmt.Fprintf(w, "Stopped periscope (pid %d).\n", rec.PID)
...
func runServeStatus(cfg config.Config) {
	...
	if IsDaemonStarting(cfg.DataDir) {
		fmt.Println("periscope is starting up.")
	...
	if recs := liveDaemonRecords(cfg.DataDir); len(recs) > 0 {
		fmt.Printf(
			"periscope process running (pid %d) but not responding "+
				"to health checks.\n",
			recs[0].PID,
		)
		return
	}
	fmt.Println("No periscope server is running.")
}

// serveStatusLines renders the human-readable status of a discovered daemon.
func serveStatusLines(rt *DaemonRuntime) []string {
	lines := []string{
		fmt.Sprintf("periscope running at %s", urlFromDaemonRuntime(rt)),
...
func serveIncompatibleDaemonStatusLines(
	...
	return append(lines,
		"Run `periscope daemon restart` to replace it, or "+
			"`periscope daemon stop` to stop it first.",
	)
}

// runServeStop terminates every periscope server owning this data dir whose
...
func runServeStop(cfg config.Config) {
	...
	for _, rec := range records {
		if !stopTargetConfirmed(rec, cfg.AuthToken) {
			fmt.Printf(
				"Skipping pid %d: cannot confirm it is the recorded "+
					"periscope daemon (stale record or reused pid).\n",
				rec.PID,
			)
			skipped++
			continue
		...
		fmt.Printf("Stopped periscope (pid %d).\n", rec.PID)
		stopped++
	}
	if stopped == 0 && skipped > 0 {
		fmt.Println(
			"No periscope server was stopped; runtime records may be stale.",
		)
	}
}
...
func stopDaemonRuntimeForUpgradeImpl(
	...
	if !stopTargetConfirmed(rt.Record, cfg.AuthToken) {
		return fmt.Errorf(
			"cannot confirm pid %d is the recorded periscope daemon",
			rt.Record.PID,
		)
	}
...
func stopWritableDaemonsForUpdate(
	...
	if !result.Stopped && IsDaemonStarting(cfg.DataDir) {
		return result, fmt.Errorf(
			"periscope server is starting; retry the update once it is ready",
		)
	}
	return result, nil
}
...
// recorded periscope daemon. It accepts the target when the daemon answers the
...
// probe as the periscope daemon it claims to be.
