...
func TestRecallImportRefusesExplicitServerURLWithoutRemoteConfirmation(t *testing.T) {
	...
	input := `{"candidate_id":"m-remote-import","type":"debugging_method","scope":"repository","title":"Check cwd before file reads","body":"Verify cwd before retrying failed reads.","project":"periscope","agent":"codex","session_id":"recall-session","label":"correct","transferable":true,"provenance_ok":true,"evidence":{"ordinal_start":3,"ordinal_end":7}}
`
...
func TestRecallImportHelpDescribesProductionOverrideForAnyDefaultArchive(t *testing.T) {
	...
	assert.Contains(t, out, "default periscope data directory")
	assert.NotContains(t, out, "default local periscope data directory")
}
...
func TestRecallBriefCurrentWorktreeScopesToGitRootAndBranch(t *testing.T) {
	...
	subdir := filepath.Join(repo, "cmd", "periscope")
...
func TestRecallImportJSONLImportsReviewedKeepers(t *testing.T) {
	...
	seedRecallEntryFixture(t, dataDir)
	path := filepath.Join(t.TempDir(), "accepted-recall.jsonl")
	input := `{"candidate_id":"m-imported","type":"debugging_method","scope":"repository","title":"Check cwd before file reads","body":"Verify cwd before retrying failed reads.","project":"periscope","agent":"codex","session_id":"recall-session","label":"correct","transferable":true,"provenance_ok":true,"evidence":{"ordinal_start":3,"ordinal_end":7}}
{"candidate_id":"m-rejected","type":"fact","scope":"project","title":"Rejected","body":"Rejected.","project":"periscope","agent":"codex","session_id":"recall-session","label":"wrong","transferable":false,"provenance_ok":false,"evidence":{"ordinal_start":1,"ordinal_end":1}}
`
	require.NoError(t, os.WriteFile(path, []byte(input), 0o600))
...
func TestRecallImportJSONLRequiresExistingEvidenceByDefault(t *testing.T) {
	...
	setRecallTestEnv(t, dataDir)
	path := filepath.Join(t.TempDir(), "accepted-recall.jsonl")
	input := `{"candidate_id":"m-missing-session","type":"debugging_method","scope":"repository","title":"Check cwd before file reads","body":"Verify cwd before retrying failed reads.","project":"periscope","agent":"codex","session_id":"s-not-imported","label":"correct","transferable":true,"provenance_ok":true,"evidence":{"ordinal_start":3,"ordinal_end":7}}
`
	require.NoError(t, os.WriteFile(path, []byte(input), 0o600))
...
func TestRecallImportJSONLAllowPlaceholderSessionsImportsMissingSession(t *testing.T) {
	...
	setRecallTestEnv(t, dataDir)
	path := filepath.Join(t.TempDir(), "accepted-recall.jsonl")
	input := `{"candidate_id":"m-placeholder","type":"debugging_method","scope":"repository","title":"Check cwd before file reads","body":"Verify cwd before retrying failed reads.","project":"periscope","agent":"codex","session_id":"s-placeholder","label":"correct","transferable":true,"provenance_ok":true,"evidence":{"ordinal_start":3,"ordinal_end":7}}
`
	require.NoError(t, os.WriteFile(path, []byte(input), 0o600))
...
func TestRecallImportJSONLDryRunDoesNotInsert(t *testing.T) {
	...
	seedRecallEntryFixture(t, dataDir)
	path := filepath.Join(t.TempDir(), "accepted-recall.jsonl")
	input := `{"candidate_id":"m-dry-run","type":"debugging_method","scope":"repository","title":"Check cwd before file reads","body":"Verify cwd before retrying failed reads.","project":"periscope","agent":"codex","session_id":"recall-session","label":"correct","transferable":true,"provenance_ok":true,"evidence":{"ordinal_start":3,"ordinal_end":7}}
{"candidate_id":"m-rejected","type":"fact","scope":"project","title":"Rejected","body":"Rejected.","project":"periscope","agent":"codex","session_id":"recall-session","label":"wrong","transferable":false,"provenance_ok":false,"evidence":{"ordinal_start":1,"ordinal_end":1}}
`
	require.NoError(t, os.WriteFile(path, []byte(input), 0o600))
...
func TestRecallImportJSONLDryRunHumanShowsPreviewAndSkippedReasons(t *testing.T) {
	...
	seedRecallEntryFixture(t, dataDir)
	path := filepath.Join(t.TempDir(), "accepted-recall.jsonl")
	input := `{"candidate_id":"m-dry-run","supersedes_entry_id":"m-cli","type":"debugging_method","scope":"repository","title":"Check cwd before file reads","body":"Verify cwd before retrying failed reads.","project":"periscope","agent":"codex","session_id":"recall-session","label":"correct","transferable":true,"provenance_ok":true,"evidence":{"ordinal_start":3,"ordinal_end":7}}
{"candidate_id":"m-rejected","type":"fact","scope":"project","title":"Rejected","body":"Rejected.","project":"periscope","agent":"codex","session_id":"recall-session","label":"wrong","transferable":true,"provenance_ok":true,"evidence":{"ordinal_start":1,"ordinal_end":1}}
`
	require.NoError(t, os.WriteFile(path, []byte(input), 0o600))
