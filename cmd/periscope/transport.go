...
// ABOUTME: periscope daemon is discoverable via its kit runtime record.
package main
...
// CLI test reaching this path would detach a real `periscope.test serve`
		...
		if daemonAutostartDisabled() {
			return transport{}, errors.New(
				"daemon autostart is disabled; direct SQLite reads are " +
					...
					"`periscope daemon start` or unset " +
...
func guardDaemonAutoStartConfig(cfg config.Config) error {
	...
	return fmt.Errorf(
		"refusing to auto-start an unauthenticated daemon on non-loopback "+
			...
			"start 'periscope serve --background' explicitly, or set "+
...
func directIncompatibleDaemonError(tr transport) error {
	...
	if reason == "" {
		reason = "local daemon is incompatible with this periscope client"
	}
