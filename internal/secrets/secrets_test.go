...
func TestScanFindsAWSAccessKey(t *testing.T) {
	...
	if text[m.Start:m.End] != "AKIA3VBMK8XJZ6WPCNQH" {
		t.Errorf("span = %q, want AKIA3VBMK8XJZ6WPCNQH", text[m.Start:m.End])
	}
	if m.Index != 0 {
...
func TestRedactMasksSecretButKeepsContext(t *testing.T) {
	text := "export AWS_KEY=AKIA3VBMK8XJZ6WPCNQH then continue"
	got := Redact(text)
	if strings.Contains(got, "AKIA3VBMK8XJZ6WPCNQH") {
	...
	if !strings.Contains(got, "AKIA…CNQH") {
