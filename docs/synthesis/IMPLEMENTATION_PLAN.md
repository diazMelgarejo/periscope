...
- [ ] `go.mod` / `go.sum` → `github.com/latentsignal-org/periscope`
- [ ] `cmd/periscope/` binary name
- [ ] `scripts/install.sh`, `scripts/sync-upstream.sh`, release CI
- [ ] `scripts/git/verify-staged-for-commit.sh` (orama superset)
- [ ] `PERISCOPE_OWNED` list in sync-upstream.sh

**Extend:** sync-upstream.sh kenn-io Layer 1 auto-resolution rules

**Verify:** `grep -r wesm/agentsview --include='*.go' .` → zero; `go build ./cmd/periscope`
...
- [ ] Keep `frontend/src/lib/components/context/` from merged (Layer 2)
- [ ] Take kit-ui migration from purified PR #26 where non-conflicting
- [ ] Union `frontend/src/lib/stores/`, router changes
...
- [ ] Union `docs/` — keep merged periscope guides + purified upstream fixes
- [ ] ECC: replay only harmonized paths (SKILL.md mirrors, instincts) — skip timestamp-only JSON
- [ ] Update synthesis progress in this file
...
| 3 | pending | — | pass3-agent | — |
| 4 | pending | — | pass4-agent | — |
| 5 | pending | — | pass5-agent | — |
